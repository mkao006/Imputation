...
\frame{
  ...
  \begin{align}
    \label{eq:lmeImpute}
  \text{Y}_{i,t} &= \overbrace{\beta_{0j} + \beta_{1j}t + b_{2,i,
      t}\bar{Y}_{j,t}}^{\text{Fixed effect}} + \overbrace{b_{0,i} +
...
\frame{
  \includegraphics[page = 54, scale = 0.4]{checkWheatImputation}
}

\frame{
  \includegraphics[page = 92, scale = 0.4]{checkWheatImputation}
}

\frame{
  \includegraphics[page = 109, scale = 0.4]{checkWheatImputation}
}


\frame{
  \includegraphics[page = 115, scale = 0.4]{checkWheatImputation}
}
...
\frame{
  What we have presented is the methodology, now we will present how the
  methodology will be integrated into the Food Balance Sheet cycle.
...
\frame{
  ...
  the wiki page at the following repository.
