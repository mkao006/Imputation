...
\frame{
  ...
  First of all, we propose to impute the yield and area, and along with
...
\frame{
  ...
  The average yield can be calculated as:
  ...
  Which acts as a proxy to reflect the change in climatics conditions and
  other factors which can simultaneously affect multiple countries.  
...
\frame{
  ...
  \textbf{last observation carry forward} to impute area harvestd.
  
  \vspace{0.5cm}

  First the area harvested displays close to monotonic behaviour and
  much little year-to-year fluctuation and thus linear interpolation
  is suitable. Furthermore, a previous simulation study has shown linear
  interpolation gives best result.
...
\subsection{Results for Wheat}

\frame{
  \includegraphics[page = 5, scale = 0.4]{checkWheatImputation}
}


\frame{
  \includegraphics[page = 13, scale = 0.4]{checkWheatImputation}
}
...
\begin{itemize}
  \item Test multilevel structured additive regression to account for
    non-linearity in yield.
  \item Test EM-algorithm to estimate rather than compute the
    sub-regional average yield.
  \item Understand and improve the methodology for area harvested.
\end{itemize}
