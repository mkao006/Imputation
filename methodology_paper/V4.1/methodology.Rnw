...
read.example = function(file){
    ...
    removeNoInfo(tmp2, value = "yieldValue", flag = "yieldSymb",
                 byKey = "areaCode")
...
<<beef-yield-explore, fig.width=15, fig.height=12, fig.cap='Similar to grape, the carcass weight per animal of beef also shows the same pattern of a very modest improvement in yield with a handful of exceptions.', fig.pos='!ht'>>=
...
<<wheat-production-area-explore, fig.width=15, fig.height=15, fig.cap='Wheat production and area harvested by country. The figure shows that excluding several producers such as Turkemenistan, Nepal, and Pakistan which has a stable trend in production, both the production and area display erratic behavior.', fig.pos='!ht'>>=
...
<<okra-production-area-explore, fig.width=15, fig.height=15, fig.cap='Even more so than both wheat and grape, the production appears to demonstrate unpredictable trends and shocks.', fig.pos='!ht'>>=
...
<<wheat-swaziland-impute, fig.width=15, fig.height=8, fig.cap='This selected series is the production of wheat in Swaziland, from the black observed production we can see that no data has been observed in the paste 10 years. Yet, the ensemble fits several possible extrapolation which all seem reasonable. Yet, we can see that although the linear regression obtained the highest weight the ensemble is safe guarded by naive interpolation and spline so it does not go below zero.', fig.pos='!ht'>>=


wheatYieldImputed.dt = copy(wheat.dt)

imputeYield(yieldValue = "yieldValue",
            yieldObservationFlag = "yieldSymb",
            yieldMethodFlag = "yieldSymb2",
            yearValue = "year",
            imputationFlag = "I",
            newMethodFlag = "e",
            byKey = "areaCode",
            data = wheatYieldImputed.dt,
            yieldFormula = yieldValue ~ -1 + (1 + bs(yearValue, df = 2, degree = 1)|areaCode))


with(wheatYieldImputed.dt[areaName == "Swaziland", ],
     ensembleImpute(productionValue, plot = TRUE)
     )

@ 

<<wheat-madagascar, fig.width=15, fig.height=8, fig.cap='In contrast to swaziland, the wheat production in Mauritania exhibits a simple shape. Almost equal weights were allocated to model which did not fail.', fig.pos='!ht'>>=

with(wheat.dt[areaName == "Madagascar", ],
     ensembleImpute(productionValue, plot = TRUE)
     )
...
<<grape-zimbabwe, fig.width=15, fig.height=8, fig.cap='In this depiction, the grape of Zimbabwe is used for illustration. In the case where there is not sufficient amount of data, the ensemble will collapse to form a simple model with strong agreements between the models.', fig.pos='!ht'>>=

grapeYieldImputed.dt = copy(grape.dt)
...
imputeYield(yieldValue = "yieldValue",
            ...
            newMethodFlag = "e",
            byKey = "areaCode",
            data = grapeYieldImputed.dt,
            yieldFormula = yieldValue ~ -1 + (1 + bs(yearValue, df = 2, degree = 1)|areaCode))


with(grapeYieldImputed.dt[areaName == "Zimbabwe", ],
     ensembleImpute(productionValue, plot = TRUE)
     )
...
<<okra-ghana, fig.width=15, fig.height=8, fig.cap='There appears to be a gap in the series of the production of Okra for Iraq. Most model collapse to a straight line close to the mean of the series, but both spline and loess captured the possible downward trend which is reflected in the dent in the final ensemble. ', fig.pos='!ht'>>=


okraYieldImputed.dt = copy(okra.dt)
...
imputeYield(yieldValue = "yieldValue",
            ...
            newMethodFlag = "e",
            byKey = "areaCode",
            data = okraYieldImputed.dt,
            yieldFormula = yieldValue ~ -1 + (1 + bs(yearValue, df = 2, degree = 1)|areaCode))

with(okraYieldImputed.dt[areaName == "Iraq", ],
     ensembleImpute(productionValue, plot = TRUE)
     )
...
<<okra-barbados, fig.width=15, fig.height=8, fig.cap='Finally, a more complex example is shown here, the series is the production of Okra in Barbados. There were a few observation in the beginning of the series, then in the center of the time series the production doubled before falling to prior level. A relative complex model is required to fit such a time series, loess took most of the weight with the remaining weight assigned to other model.', fig.pos='!ht'>>=

with(okraYieldImputed.dt[areaName == "Barbados", ],
     ensembleImpute(productionValue, plot = TRUE)
     )
...
<<wheat-yield-imputed, fig.width=15, fig.height=15, fig.cap='We can see the imputation of yield in purple seems to produce reasonable values.', fig.pos='!ht'>>=

myFormula = yieldValue ~ -1 + (1 + bs(year, df = 2, degree = 1)|areaCode)
wheatAllImputed.dt =
    imputeProductionDomain(data = wheat.dt,
                           ...
                           maximumWeights = 0.5,
                           yieldFormula = myFormula)

setnames(wheatAllImputed.dt, old = colnames(wheatAllImputed.dt),
         gsub("Value", "ImputedValue", colnames(wheatAllImputed.dt)))

setnames(wheatAllImputed.dt, old = colnames(wheatAllImputed.dt),
         gsub("Symb", "NewSymb", colnames(wheatAllImputed.dt)))

wheatFinal.dt = merge(wheat.dt, wheatAllImputed.dt, 
    by = intersect(colnames(wheat.dt), colnames(wheatAllImputed.dt)))
wheatFinal.dt[!is.na(productionValue),
              productionImputedValue := NA]
wheatFinal.dt[!is.na(areaHarvestedValue),
              areaHarvestedImputedValue := NA]
wheatFinal.dt[!is.na(yieldValue),
              yieldImputedValue := NA]

xyplot(yieldValue + yieldImputedValue ~ year|areaName, 
       data = wheatFinal.dt, type = c("g", "l", "p"), cex = 0.2,
       auto.key = 
...
wheatFinal.dt[, scaledProductionValue :=
              productionValue/
              max(productionValue, productionImputedValue,
                  na.rm = !all(is.na(productionValue))),
         by = "areaName"]


wheatFinal.dt[, scaledProductionImputedValue :=
              productionImputedValue/
              max(productionImputedValue,  productionValue,
...
xyplot(scaledProductionValue + scaledProductionImputedValue ~ 
       year|areaName, 
       data = wheatFinal.dt, type = c("g", "l", "p"), cex = 0.5,
       auto.key = 
...
wheatFinal.dt[, scaledAreaHarvestedValue :=
              areaHarvestedValue/
              max(areaHarvestedValue, areaHarvestedImputedValue,
                  na.rm = !all(is.na(areaHarvestedValue))),
         by = "areaName"]


wheatFinal.dt[, scaledAreaHarvestedImputedValue :=
         areaHarvestedImputedValue/
              max(c(areaHarvestedImputedValue,  areaHarvestedValue), 
...
<<grape-yield-imputed, fig.width=15, fig.height=15, fig.cap='The imputation of yield also appears to be reasonable, even in the case for Iraq the linear mixed model is not severely influenced by the one off event.', fig.pos='!ht'>>=



myFormula = yieldValue ~ -1 + (1 + bs(year, df = 2, degree = 1)|areaCode)
grapeAllImputed.dt =
    imputeProductionDomain(data = grape.dt,
                           productionValue = "productionValue",
                           areaHarvestedValue =
                           "areaHarvestedValue",
                           yieldValue = "yieldValue",
...
setnames(grapeAllImputed.dt, old = colnames(grapeAllImputed.dt),
         gsub("Value", "ImputedValue", colnames(grapeAllImputed.dt)))

setnames(grapeAllImputed.dt, old = colnames(grapeAllImputed.dt),
         gsub("Symb", "NewSymb", colnames(grapeAllImputed.dt)))

grapeFinal.dt = merge(grape.dt, grapeAllImputed.dt, 
    by = intersect(colnames(grape.dt), colnames(grapeAllImputed.dt)))
grapeFinal.dt[!is.na(productionValue),
              productionImputedValue := NA]
grapeFinal.dt[!is.na(areaHarvestedValue),
              areaHarvestedImputedValue := NA]
grapeFinal.dt[!is.na(yieldValue),
              yieldImputedValue := NA]

xyplot(yieldValue + yieldImputedValue ~ 
       year|areaName, 
       data = grapeFinal.dt, type = c("g", "l", "p"), cex = 0.5,
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "", xlab = "", 
       scales = list(y = list(draw = FALSE)))
...
grapeFinal.dt[, scaledProductionValue :=
         productionValue/
              max(productionValue, productionImputedValue,
                  na.rm = !all(is.na(productionValue))),
         by = "areaName"]
...
grapeFinal.dt[, scaledProductionImputedValue :=
              productionImputedValue/
              max(productionImputedValue,  productionValue,
                  na.rm = !all(is.na(productionImputedValue))),
         by = "areaName"]
...
<<grape-areaharvested-imputed, fig.width=15, fig.height=15, fig.cap='Imputation of area harvested of grape. The area harvested does appear to be problematic, mostely resembling the trend of the production.', fig.pos='!ht'>>=

grapeFinal.dt[, scaledAreaHarvestedValue :=
         areaHarvestedValue/
              max(areaHarvestedValue,  areaHarvestedImputedValue,
                  na.rm = !all(is.na(areaHarvestedValue))),
         by = "areaName"]
...
grapeFinal.dt[, scaledAreaHarvestedImputedValue :=
         areaHarvestedImputedValue/
              max(c(areaHarvestedImputedValue,  areaHarvestedValue), 
                  na.rm = !all(is.na(areaHarvestedImputedValue))),
         by = "areaName"]
...
<<okra-yield-imputed, fig.width=15, fig.height=15, fig.cap='The imputation of yield also appears to be reasonable, here once again we see the robustness of linear mixed model not being influenced by the bad data quality of both Senegal and Bahrain.', fig.pos='!ht'>>=
...
setnames(okraAllImputed.dt, old = colnames(okraAllImputed.dt),
         gsub("Value", "ImputedValue", colnames(okraAllImputed.dt)))

setnames(okraAllImputed.dt, old = colnames(okraAllImputed.dt),
         gsub("Symb", "NewSymb", colnames(okraAllImputed.dt)))

okraFinal.dt = merge(okra.dt, okraAllImputed.dt, 
    by = intersect(colnames(okra.dt), colnames(okraAllImputed.dt)))
okraFinal.dt[!is.na(productionValue),
              productionImputedValue := NA]
okraFinal.dt[!is.na(areaHarvestedValue),
              areaHarvestedImputedValue := NA]
okraFinal.dt[!is.na(yieldValue),
              yieldImputedValue := NA]

xyplot(yieldValue + yieldImputedValue ~ 
       year|areaName, 
       data = okraFinal.dt, type = c("g", "l", "p"), cex = 0.5,
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "", xlab = "", 
       scales = list(y = list(draw = FALSE)))
...
<<okra-production-imputed, fig.width=15, fig.height=15, fig.cap='The imputation of okra prodution.', fig.pos='!ht'>>=

okraFinal.dt[, scaledProductionValue :=
         productionValue/
              max(productionValue, productionImputedValue,
                  na.rm = !all(is.na(productionValue))),
         by = "areaName"]
...
okraFinal.dt[, scaledProductionImputedValue :=
              productionImputedValue/
              max(productionImputedValue,  productionValue,
                  na.rm = !all(is.na(productionImputedValue))),
         by = "areaName"]
...
<<okra-areaharvested-imputed, fig.width=15, fig.height=15, fig.cap='Imputation of area harvested of okra. The area harvested of Viet Nam does appear to be problematic, the unexpectantly high area harvested is a result of an extremely low yield in the earlier years. This is due to the fact that long extrapolation for over 40 years with only approximately 10 years of data is unreasonable. We can eliminate this problem if we restrict the working data to the past 3 decades.', fig.pos='!ht'>>=

okraFinal.dt[, scaledAreaHarvestedValue :=
         areaHarvestedValue/
              max(areaHarvestedValue,  areaHarvestedImputedValue,
                  na.rm = !all(is.na(areaHarvestedValue))),
         by = "areaName"]
...
okraFinal.dt[, scaledAreaHarvestedImputedValue :=
         areaHarvestedImputedValue/
              max(c(areaHarvestedImputedValue,  areaHarvestedValue), 
                  na.rm = !all(is.na(areaHarvestedImputedValue))),
         by = "areaName"]
...
<<beef-yield-imputed, fig.width=15, fig.height=15, fig.cap='Imputation of beef carcass weight. .', fig.pos='!ht'>>=



myFormula = yieldValue ~ -1 + (1 + bs(year, df = 2, degree = 1)|areaCode)
beefAllImputed.dt =
    imputeProductionDomain(data = beef.dt,
                           productionValue = "productionValue",
                           areaHarvestedValue =
                           "areaHarvestedValue",
                           yieldValue = "yieldValue",
...
setnames(beefAllImputed.dt, old = colnames(beefAllImputed.dt),
         gsub("Value", "ImputedValue", colnames(beefAllImputed.dt)))

setnames(beefAllImputed.dt, old = colnames(beefAllImputed.dt),
         gsub("Symb", "NewSymb", colnames(beefAllImputed.dt)))

beefFinal.dt = merge(beef.dt, beefAllImputed.dt, 
    by = intersect(colnames(beef.dt), colnames(beefAllImputed.dt)))
beefFinal.dt[!is.na(productionValue),
              productionImputedValue := NA]
beefFinal.dt[!is.na(areaHarvestedValue),
              areaHarvestedImputedValue := NA]
beefFinal.dt[!is.na(yieldValue),
              yieldImputedValue := NA]

xyplot(yieldValue + yieldImputedValue ~ 
       year|areaName, 
       data = beefFinal.dt, type = c("g", "l", "p"), cex = 0.5,
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "", xlab = "", 
       scales = list(y = list(draw = FALSE)))
...
beefFinal.dt[, scaledProductionValue :=
         productionValue/
             max(productionValue, productionImputedValue,
                 na.rm = !all(is.na(productionValue))),
         by = "areaName"]
...
beefFinal.dt[, scaledProductionImputedValue :=
         productionImputedValue/
             max(productionImputedValue, productionValue,
                 na.rm = !all(is.na(productionImputedValue))),
         by = "areaName"]
...
beefFinal.dt[, scaledAreaHarvestedValue :=
         areaHarvestedValue/
             max(areaHarvestedValue,  areaHarvestedImputedValue,
                 na.rm = !all(is.na(areaHarvestedValue))),
         by = "areaName"]
...
beefFinal.dt[, scaledAreaHarvestedImputedValue :=
         areaHarvestedImputedValue/
             max(c(areaHarvestedImputedValue,  areaHarvestedValue), 
                 na.rm = !all(is.na(areaHarvestedImputedValue))),
         by = "areaName"]
