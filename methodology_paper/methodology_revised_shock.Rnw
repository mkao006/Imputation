...
wheat.dt[, deltaProduction := c(NA, diff(log(productionNum))),
             by = "areaName"]
wheat.dt[, deltaArea := c(NA, diff(log(areaNum))),
             by = "areaName"]
wheat.dt[, deltaYield := c(NA, diff(log(yieldNum))),
             by = "areaName"]
...
<<production-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Production time series by country'>>=
xyplot(log(productionNum) ~ Year|areaName, wheat.dt, type = c("g", "l"),
       ylab = "Log of production")
@ 

<<area-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Area time series by country'>>=
xyplot(log(areaNum) ~ Year|areaName, wheat.dt, type = c("g", "l"),
       ylab = "Log of area harvested")
@ 

<<yield-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Yield time series by country'>>=
