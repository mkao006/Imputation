...
## source("../codes/validateYield.R")
## source("../codes/predict.meanlme4.R")
source("../codes/predict.shocklme4.R")
## source("../codes/impBootPred.R")
...
<<data-manipulation>>=
wheat.dt =
    swsToImputationDataTable(file =
                             "../case_study/sws_data/wheatSUA.csv",
                             denormalizer = "Element.Code")
grape.dt =
    swsToImputationDataTable(file =
                             "../case_study/sws_data/grapeSUA.csv",
                             denormalizer = "Element.Code")

pepper.dt =
    swsToImputationDataTable(file =
                             "../case_study/sws_data/pepperSUA.csv",
                             denormalizer = "Element.Code")
@ 
...
<<area-production-yield, fig.width=15, fig.height=8, fig.cap='Relation between production, area harvested and yield', fig.pos='!ht'>>=

scatterplotMatrix(~log(productionNum) + log(areaNum) +
                  log(yieldNum),
                  data = wheat.dt[Year == 2011, ],
                  smoother = FALSE)
@ 

<<production-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Logged production of wheat from 1961 to 2011 by country'>>=
xyplot(log(productionNum) ~ Year|areaName, wheat.dt, type = c("g", "l"),
       ylab = "Log of production")
@ 

<<area-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Logged area harvested of wheat from 1961 to 2011 by country'>>=
xyplot(log(areaNum) ~ Year|areaName, wheat.dt, type = c("g", "l"),
       ylab = "Log of area harvested")
@ 

<<yield-time, fig.width=15, fig.height=10, fig.pos='!ht', fig.cap='Logged yield series of wheat from 1961 to 2011 by country'>>=
xyplot(log(yieldNum) ~ Year|areaName, wheat.dt, type = c("g", "l"),
       ylab = "Log of yield")
@ 
...
<<wheat-production-sparsity, fig.width=12, fig.height=9, fig.cap='Missing pattern of wheat by value of production', fig.pos='!ht'>>=
wheatSparsity.df = data.frame(splitNACountry(valueVar = "productionNum", 
    countryVar = "areaName", data = wheat.dt)$nonEmptyData)
wheatSparsity.df[which(wheatSparsity.df$productionNum == 0), 
                 "productionNum"] = NA
sparsityHeatMap(wheatSparsity.df,
                country = "areaName", year = "Year", 
                var = "productionNum", ncol = 3)

@ 


<<grape-production-sparsity, fig.width=12, fig.height=9, fig.cap='Missing pattern of grape by value of production', fig.pos='!ht'>>=
grapeSparsity.df = data.frame(splitNACountry(valueVar = "productionNum", 
    countryVar = "areaName", data = grape.dt)$nonEmptyData)
grapeSparsity.df[which(grapeSparsity.df$productionNum == 0), 
                 "productionNum"] = NA
sparsityHeatMap(grapeSparsity.df,
                country = "areaName", year = "Year", 
                var = "productionNum", ncol = 3)
...
<<wheat-data-description, results='asis'>>=
wheatFlagTable = 
    cbind(table(wheat.dt[Year %in% c(1994:2011),  
              productionSymb], useNA = "always"),
          table(wheat.dt[Year %in% c(1994:2011), productionSymb],
                useNA = "always")/
          NROW(wheat.dt[Year %in% c(1994:2011), productionSymb]),
          table(wheat.dt[Year %in% c(1994:2011), areaSymb], 
                useNA = "always"),
          table(wheat.dt[Year %in% c(1994:2011), areaSymb],
                useNA = "always")/
          NROW(wheat.dt[Year %in% c(1994:2011), areaSymb]))
rownames(wheatFlagTable)[is.na(rownames(wheatFlagTable))] = "Missing"
wheatFlagTable = data.frame(rbind(wheatFlagTable, 
    Total = colSums(wheatFlagTable)))

wheatFlagTable[, 2] = paste0(round(wheatFlagTable[, 2], 2) * 100,
                  "%")
wheatFlagTable[, 4] = paste0(round(wheatFlagTable[, 4], 2) * 100,
                  "%")
colnames(wheatFlagTable) = c("Production Count", 
            "Production Percentage(%)", "Area Count", 
            "Area Percentage (%)")
xtable(wheatFlagTable, 
       captions = "Flag table for wheat production domain",
       digits = 0, align = "|l|c|c|c|c|")
@ 

<<wheat-imputation>>=
wheatImputed.lst = swsImputation(wheat.dt[Year %in% c(1994:2011), ], 
    area = "areaNum", prod = "productionNum", yield = "yieldNum",
...
<<wheat-obs-fitted-yield-time, fig.width=15, fig.height=10, fig.cap='Observed yied versus fitted yield. Here the pink line represents official or semi-official data while the blue line represents the fit to the data. Empty boxes implies that no yield was observed and imputed for that country.', fig.pos='!ht'>>=
xyplot(fittedYield + yieldNum ~ Year|areaName, 
       data = wheatImputed.lst$imputed,
...
<<wheat-obs-fitted-production-time, fig.width=15, fig.height=10, fig.cap='Observed production versus imputed production. In order to make the countries comparable, we have log transformed the data.', fig.pos='!ht'>>=
xyplot(log(imputedProd) + log(productionNum) ~ Year|areaName, 
       data = wheatImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed logged production", 
       xlab = "Year")
@ 

<<wheat-obs-fitted-area-time, fig.width=15, fig.height=10, fig.cap='Observed area versus imputed area. Similar to the production, the area is log transformed.', fig.pos='!ht'>>=
xyplot(log(imputedArea) + log(areaNum) ~ Year|areaName, 
       data = wheatImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed log area", xlab = "Year")
@ 
...
<<grape-data-description, results='asis'>>=
grapeFlagTable = 
    cbind(table(grape.dt[Year %in% c(1994:2011),  
              productionSymb], useNA = "always"),
          table(grape.dt[Year %in% c(1994:2011), productionSymb],
                useNA = "always")/
          NROW(grape.dt[Year %in% c(1994:2011), productionSymb]),
          table(grape.dt[Year %in% c(1994:2011), areaSymb], 
                useNA = "always"),
          table(grape.dt[Year %in% c(1994:2011), areaSymb],
                useNA = "always")/
          NROW(grape.dt[Year %in% c(1994:2011), areaSymb]))
rownames(grapeFlagTable)[is.na(rownames(grapeFlagTable))] = "Missing"
grapeFlagTable = data.frame(rbind(grapeFlagTable, 
    Total = colSums(grapeFlagTable)))

grapeFlagTable[, 2] = paste0(round(grapeFlagTable[, 2], 2) * 100,
                  "%")
grapeFlagTable[, 4] = paste0(round(grapeFlagTable[, 4], 2) * 100,
                  "%")
colnames(grapeFlagTable) = c("Production Count", 
            "Production Percentage(%)", "Area Count", 
            "Area Percentage (%)")
xtable(grapeFlagTable, 
       captions = "Flag table for grape production domain",
       digits = 0, align = "|l|c|c|c|c|")
@ 


<<grape-imputation>>=
grapeImputed.lst = swsImputation(grape.dt[Year %in% c(1994:2011), ], 
    area = "areaNum", prod = "productionNum", yield = "yieldNum",
    country = "areaName", region = "unsdSubReg",
    year = "Year", tol = 1e-3, EMverbose = FALSE,
    meanType = "shocklme4")
...
<<grape-obs-fitted-yield-time, fig.width=15, fig.height=10, fig.cap='Observed yied versus fitted yield', fig.pos='!ht'>>=
xyplot(fittedYield + yieldNum ~ Year|areaName, 
       data = grapeImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Fitted value VS Observed", xlab = "Year")
@ 
...
<<grape-obs-fitted-production-time, fig.width=15, fig.height=10, fig.cap='Observed production versus imputed production', fig.pos='!ht'>>=
xyplot(log(imputedProd) + log(productionNum) ~ Year|areaName, 
       data = grapeImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed logged production", 
...
<<grape-obs-fitted-area-time, fig.width=15, fig.height=10, fig.cap='Observed area versus imputed area', fig.pos='!ht'>>=
xyplot(log(imputedArea) + log(areaNum) ~ Year|areaName, 
       data = grapeImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed logged area", xlab = "Year")
@ 
...
<<pepper-data-description, results='asis'>>=
pepperFlagTable = 
    cbind(table(pepper.dt[Year %in% c(1994:2011),  
              productionSymb], useNA = "always"),
          table(pepper.dt[Year %in% c(1994:2011), productionSymb],
                useNA = "always")/
          NROW(pepper.dt[Year %in% c(1994:2011), productionSymb]),
          table(pepper.dt[Year %in% c(1994:2011), areaSymb], 
                useNA = "always"),
          table(pepper.dt[Year %in% c(1994:2011), areaSymb],
                useNA = "always")/
          NROW(pepper.dt[Year %in% c(1994:2011), areaSymb]))
rownames(pepperFlagTable)[is.na(rownames(pepperFlagTable))] = "Missing"
pepperFlagTable = data.frame(rbind(pepperFlagTable, 
    Total = colSums(pepperFlagTable)))

pepperFlagTable[, 2] = paste0(round(pepperFlagTable[, 2], 2) * 100,
                  "%")
pepperFlagTable[, 4] = paste0(round(pepperFlagTable[, 4], 2) * 100,
                  "%")
colnames(pepperFlagTable) = c("Production Count", 
            "Production Percentage(%)", "Area Count", 
            "Area Percentage (%)")
xtable(pepperFlagTable, 
       captions = "Flag table for pepper production domain",
       digits = 0, align = "|l|c|c|c|c|")
@ 

<<pepper-imputation>>=
pepperImputed.lst = swsImputation(pepper.dt[Year %in% c(1994:2011), ], 
    area = "areaNum", prod = "productionNum", yield = "yieldNum",
    country = "areaName", region = "unsdSubReg",
    year = "Year", tol = 1e-3, EMverbose = FALSE,
    meanType = "shocklme4")
...
<<pepper-obs-fitted-yield-time, fig.width=15, fig.height=10, fig.cap='Observed yied versus fitted yield', fig.pos='!ht'>>=
xyplot(fittedYield + yieldNum ~ Year|areaName, 
       data = pepperImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Fitted value VS Observed", xlab = "Year")
@ 

<<pepper-obs-fitted-production-time, fig.width=15, fig.height=10, fig.cap='Observed production versus imputed production', fig.pos='!ht'>>=
xyplot(log(imputedProd) + log(productionNum) ~ Year|areaName, 
       data = pepperImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed logged production", 
...
<<pepper-obs-fitted-area-time, fig.width=15, fig.height=10, fig.cap='Observed area versus imputed area', fig.pos='!ht'>>=
xyplot(log(imputedArea) + log(areaNum) ~ Year|areaName, 
       data = pepperImputed.lst$imputed,
                 aspect = "fill", type = c("g", "l"), auto.key = TRUE,
                 ylab = "Imputed VS Observed logged area", xlab = "Year")
...
(\Sexpr{length(unique(wheat.dt[!is.na(productionNum), areaName]))})
and grape (\Sexpr{length(unique(grape.dt[!is.na(productionNum),
    areaName]))}) is much larger than pepper
(\Sexpr{length(unique(pepper.dt[!is.na(productionNum), areaName]))})
...
The main take away message for the simulation result is that the
greater the number of producing countries the more information we can
utilize; resulting in higher coverage rate and better prediction
error. 

It also demonstrates extreme robustness and the prediction error
...
\subsection*{Wheat}
<<wheat-obs-fitted, fig.width=15, fig.height=10, fig.cap='Observed versus fitted value', fig.pos='!ht'>>=
xyplot(fittedYield ~ yieldNum|areaName,
       data = wheatImputed.lst$imputed,
       panel = function(x, y) {
...
<<wheat-res-fitted, fig.width=15, fig.height=10, fig.cap='Residuals versus fitted value by country', fig.pos='!ht'>>=
xyplot(res ~ fittedYield|areaName, data = wheatImputed.lst$imputed,
...
<<wheat-res-normality, fig.width=15, fig.height=10, fig.cap='Normality of residuals by country', fig.pos='!ht'>>=
qqmath(~res|areaName, data = wheatImputed.lst$imputed,
...
\subsection*{Grape}
<<grape-obs-fitted, fig.width=15, fig.height=10, fig.cap='Observed versus fitted value', fig.pos='!ht'>>=
xyplot(fittedYield ~ yieldNum|areaName,
       data = grapeImputed.lst$imputed,
       panel = function(x, y) {
           panel.xyplot(x, y, cex = 0.5)
...
<<grape-res-fitted, fig.width=15, fig.height=10, fig.cap='Residuals versus fitted value by country', fig.pos='!ht'>>=
xyplot(res ~ fittedYield|areaName, data = grapeImputed.lst$imputed,
       type = c("g", "p"), auto.key = TRUE,
       panel = function(x, y){
           panel.xyplot(x, y)
           panel.abline(h = 0)
...
<<grape-res-normality, fig.width=15, fig.height=10, fig.cap='Normality of residuals by country', fig.pos='!ht'>>=
qqmath(~res|areaName, data = grapeImputed.lst$imputed,
       prepanel = prepanel.qqmathline,
                 panel = function(x, ...) {
                     panel.qqmathline(x, ...)
                     panel.qqmath(x, ...)
