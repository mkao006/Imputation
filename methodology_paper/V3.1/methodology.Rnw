...
increasing trend in the yield accross all country and commodity
illustrated. Similar stories are observed in almost all commodity that
has been studied during the development of the methodology. This is a
result of continuous advancement in both technology and agricultural
practice driven by research \& development. Improved irrigation
provides crop with sufficient and uninterrupted water source, while
tailored compound feed provide the precise nutrient requirements
ensuring the livestocks consume the optimal diet for
...
Nevertheless, just like all available technology such as internet and
information, the distribution is far from perfect. The adoption of
technology depends on the access which may be hindered by the presence
of patents, or it may be restricted by service providers. Imperfect
information and limit financial resources are also major obstacles for
embracing the new developments, this is particularly true for
countries where the majority of the producers are smallholders or
rural farming.\\
...
such as the Netherlands. Inovations required are also different for
...
producer. Production can increase or decrease to reflect the state of
the market, wheat field can be substitue to harvest sorghum if prices
are expected to be high. Markets has been known to be difficult to
forecast, let alone the prediction of human judgement is just shy of
impossible.\\
...
The following plots shows the imputation in practice, in this
illustration we have only utilize cross-country information as it is
easier to visualize. 

<<wheat-yield-impute, fig.width=15, fig.height=15, fig.cap='We can see the imputation in purple seems to produce descent values. There is a small kink for Slovakia and Serbia \& Montenegro, but this should improve with within-country effect.', fig.pos='!ht'>>=

wheat.dt[, yieldImputedValue := yieldValue]
wheatYieldImputed.dt = 
    imputeYield(formula = yieldImputedValue^0.5 ~ -1 + 
                (1 + bs(year, degree = 1, df = 3)|areaName),
                data = wheat.dt, index = "areaName", weights = NULL)
wheatYieldImputed.dt[!is.na(yieldValue), 
                     yieldImputedValue := as.numeric(NA)]
wheatYieldImputed.dt[, yieldImputedValue := yieldImputedValue^2]


xyplot(yieldValue + yieldImputedValue ~ year|areaName, 
       data = wheatYieldImputed.dt, type = c("g", "l"),
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "Wheat Yield (1000 Hg/Ha)", xlab = "")
@ 

<<grape-yield-impute, fig.width=15, fig.height=15, fig.cap='The imputation for grape also seems to be acceptable, except the instance for Saudi Arabia. Nevertheless, the process of correcting the data should be the case rather than modifying the model to fit the data.', fig.pos='!ht'>>=

grape.dt[, yieldImputedValue := yieldValue]
grapeYieldImputed.dt = 
    imputeYield(formula = yieldImputedValue^0.5 ~ -1 + 
                (1 + bs(year, degree = 1, df = 3)|areaName),
                data = grape.dt, index = "areaName", weights = NULL)
grapeYieldImputed.dt[!is.na(yieldValue), 
                     yieldImputedValue := as.numeric(NA)]
grapeYieldImputed.dt[, yieldImputedValue := yieldImputedValue^2]


xyplot(yieldValue + yieldImputedValue ~ year|areaName, 
       data = grapeYieldImputed.dt, type = c("g", "l"),
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "Grape Yield (1000 Hg/Ha)", xlab = "")
@ 

<<beef-yield-impute, fig.width=15, fig.height=15, fig.cap='Again, the imputation of missing value for beef also seems reasonable. This is largely to due to simple nature of yield.', fig.pos='!ht'>>=

beef.dt[, yieldImputedValue := yieldValue]
beefYieldImputed.dt = 
    imputeYield(formula = yieldImputedValue^0.5 ~ -1 + 
                (1 + year|areaName), data = beef.dt, 
                index = "areaName", weights = NULL)
beefYieldImputed.dt[!is.na(yieldValue), 
                     yieldImputedValue := as.numeric(NA)]
beefYieldImputed.dt[, yieldImputedValue := yieldImputedValue^2]


xyplot(yieldValue + yieldImputedValue ~ year|areaName, 
       data = beefYieldImputed.dt, type = c("g", "l"),
       auto.key = 
       list(text = c("Observed Yield", "Imputed Yield")),
       ylab = "Beef Yield (1000 Hg/Ha)", xlab = "")
...
Where the weights ($w_i$) of model $i$ depends on its fit ($f_{n,
  i}(x)$) on the available data. The naive imputation which does not
have a fitted model always take uniform weights. A much more
favourable method is to compute weights based on the prediction error
obtained from bootstrap or jackknife, yet the missing values restrict
us to draw any reasonable size for sub-sample fitting. This is further
penalized by the time series nature of the data where boostrap with
