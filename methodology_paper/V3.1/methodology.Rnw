...
grape.dt = foo("grapesSUA.csv")
pea.dt = foo("peasGreenSUA.csv")
...
We first visualize the data to grasp an understanding of the pattern
in our data and the relationship between the variables.

\subsection{Yield}

We first explore the pattern in the yield, the next three graph
depicts the yield of three selected commodity, wheat, grape and peas.\\


From the graph, we can observe that there is a general increasing
trend in the yield accross all country and commodity illustrated. This
...
source, while tailored compound feed provide the specific nutrients
requirement ensure livestocks consume the optimal diet for
...
Nevertheless, just like all available technology and information, the
...
may be restricted by market distribution. Imperfect information and
limit financial resources are also major obstacles embracing the new
developments, this is particularly true for countries which has a
large proportion of rural farming or smallholder.\\

Furthermore, countries faces different constraints and cost. Countries
such as Brazil and Russia which has a large amount of arable land does
not face the same pressure to improve yield compare to small states
...
these factor generate contribute towards a positive result which can
be estimated as an aggregated mixture effect.\\
...
<<pea-yield-explore, fig.width=15, fig.height=12, fig.cap='pea yield plotted by country', fig.pos='!ht'>>=

xyplot(yieldValue ~ year|areaName, data = pea.dt, type = c("g", "l"),
       xlab = "", ylab = "Green Pea Yield (1000 Hg/Ha)")
...
<<beef-yield-explore, fig.width=15, fig.height=12, fig.cap='Similar to wheat, the carcass weight per animal of beef also shows the same pattern where improved aggricultural practice raises the productivity. In particularly in countries such as Japan, United States of America and Australia where commercialization is high.', fig.pos='!ht'>>=
...
actual quantity of production is usually dectated by the area sown and
...
perception and expectation of the information leading to judgement of
the producer. Production can increase or decrease to reflect the state
of the market, wheat field can be substitue to harvest sorghum if
prices are expected to be high. Markets has been known to be difficult
to forecast, let alone the prediction of human judgement is just shy
of impossible.\\
...
demand. However in the case where the commodity is of lesser
...
<<pea-production-area-explore, fig.width=15, fig.height=15, fig.cap='Production and area harvested of pea plotted by country', fig.pos='!ht'>>=

xyplot(productionValue + areaHarvestedValue ~ year|areaName, 
       data = pea.dt, type = c("g", "l"),
       xlab = "", ylab = "", 
       scales = list(y = list(relation = "free", draw = FALSE)),
       auto.key = 
       list(text = c("Production", "Area harvested")))
...
<<beef-production-area-explore, fig.width=15, fig.height=15, fig.cap='Of all the production, livestock meat such as beef and veal may have been the easiest to predict and impute. There is continuing demand around the world for meat, while shift in production is usually difficult due to the high expenditure in machinery capital. With this being said, we can liewise observe shocks and or period of contraction or expansion over time.', fig.pos='!ht'>>=
...
One worthnoting point from the exploratory analysis is that production
which display simple pattern are typically commodities in countries
which are highly commercialized or important for consumption as staple
or trade for financial resources. These series are rarely the one we
need to impute as they are generally complete, it is commodity of
lesser importance that are required to be imputed and as a result that
are often harder due to their unstable demand.\\
...
to impute missing values, but one may be shocked at the sparsity of
...
recorded. It is not uncommen to observe suspicious trends or shocks of
area harvested which results in unreasonable yield. The yield of grape
in Saudia Arabia in figure \ref{fig:grape-yield-explore} is a prime
example. Coconut production of China in 2008 is another example, a
change in classification resulted in large escalation of production
while the area harvested remained similar to the previous year
resulted in a three-fold increase in the yield in the particular
year.\\
...
both historical and cross-sectional to be incorporated. In addition,
...
The determination of yiel is believed to consists of two components, a
trend which reflects improvement in technoglogical innovation and
fluctuation as a result of climate related factors. \\
...
trend while cyclical-catastrophical phenomenon such el nino will
...
cycle. Shocks are difficult to model due to the number of events
observed in the history is small and the effect of each event may be
different.\\

The trend can be well modelled by the linear mixed model as it
captures the country-specific technological improvement in the
commodity but at the same time the relative improvements of the
commodity specific improvements around the globe.\\
...
offers vast variety but at the same time that the relationship maybe
too complex to be expressed in a suncinct way. This contrast to the
nature of yield where cross-country and cross-commodity information
...
judgement are relatively unpredicatable which resulted in the
difficulty of imputation of area harvested and production.\\

%% In addition, a single model such as ARIMA may fail to fit as in
%% certain cases only a few observations are available and often they may
%% not be continual.\\
...
impute the production and to deal with the multi-hypothesis problem. A
single model or methodology such as ARIMA will not provide a solution
to the multi-problem we are facing.\\

\subsubsection{The algorithm}
Ensemble learning in its simplest sense, is to build multiple models
or learners which are later combine to obtain the final model or
prediction. One of the most famous application was the prediction of
movie rating competition held by Netflix, which the top two performer
both used a form of ensemble. \\
...
There are many interpretation of why ensemble works, some suggests
that all models are wrong but some are useful and by combining these
useful model you will be able to obtain a better model. \\
...
The application of ensemble solves three main problems in our
task. First of all, all reasonable candidate model can be utilized and
weighted against the data supporting the model. Second of all, the
series may have a complex structure which can not be represented by a
standard model but can be approximated by a combination of the
model. Finally, the algorithm is adaptive in the sense that if the
data generating mechanism changes in the future, the method will shift
weights to the model which better present the data and hence the name
learning.\\
...
\begin{itemize}
  \item {\textbf{Base learners:}}
    \begin{itemize}
      ...
      \item [ARIMA:] Autoregressive Integrated Moving Average selected
        based on the based BIC.
      \item [LOWESS:] Local regression with first degree local
  ...
  \item {\textbf{Combiner:}}
    ...
    Type I: Standard
    \begin{align}
      p_n(x) &= \sum_{i=1}^N w_i f_{n, i}(x) \nonumber \\
      w_i &= \frac{\sum_{n}(1/|f_{n, i}(x) - x|)^2}{\sum_{i}\sum_{n}(1/|f_{n, i}(x) - x|)^2}\nonumber
    \end{align}
...
Where the weights ($w_i$) depends on the fit ($f_{n, i}(x)$) of model
$i$ on the available data. A much more favourable method is to compute
...
size for sub-sample fitting. This is further penalized by the fact the
time series nature of the data where boostrap with replacement is not
possible.\\
...
to maintain and specific design set up to be used in different
imputation setting.\\
...
<<wheat-swaziland-impute, fig.width=15, fig.height=8, fig.cap='This selected series is the production of wheat in Swaziland, from the black observed production we can see it display the typical erratic behaviour seen previously. Due to the expansion and contraction of production over different period, flexible model such as LOESS and ARIMA has the higher weights to accommodate the change over time.', fig.pos='!ht'>>=
with(wheat.dt[areaName == "Swaziland", ],
     ensembleImpute(productionValue, plot = TRUE)
     )
@ 

<<wheat-mauritania, fig.width=15, fig.height=8, fig.cap='In contrast to swaziland, the wheat production in Mauritania exhibits a simple shape which closely resembles piece wise linear and thus the MARS obtained the highest weight compare to LOESS in the previous case, while ARIMA provids the main imputation.', fig.pos='!ht'>>=
...
<<grape-kuwait, fig.width=15, fig.height=8, fig.cap='The production of grape in Kuwai shows an increase in production since data available until the invasion in 1990 where maybe no production exist. This is then followed by a growth since the 2000s. The value imputed during the period of war is at doubt and the effect of the war is unknown, nonetheless the ensemble provide a good imputation with the data given.', fig.pos='!ht'>>=
...
<<beef-cameroon, fig.width=15, fig.height=8, fig.cap='Finally, we show the production of beef in Cameroon where pratically all model agrees that a linear trend suffice. This demonstrates that the ensemble will select the model which is best supported by the data.', fig.pos='!ht'>>=
...
have extremely sparse data we can still fall back to the model with
the least assumption.\\
...
Schumihuber, Nicolas Sakoff, Onno Hoffmeister, Luigi Castaldi, and
...
\begin{algorithm}[H]
  ...
  \Begin{
      \ForAll{commodities}{
        ...
        \ForAll{imputed area $\hat{P}_{i, t}$}{ \If{$\hat{Y}_{i, t}
